...
class Mesh3DWidget(QtWidgets.QWidget):
    ...
    def resizeMesh(self, newRowCount, newColumnCount):
        # Remove all existing series
        for series in self.surface.seriesList():
            self.surface.removeSeries(series)

        # Create the data array
        self.data = [[QtDataVisualization.QSurfaceDataItem(QtGui.QVector3D(c, 0, r)) for c in range(newColumnCount)] for r in range(newRowCount)]
        ...
        self.surface.axisX().setSegmentCount(max(1, newColumnCount - 1))
        self.surface.axisX().setLabelFormat('%i')
        self.surface.axisZ().setSegmentCount(max(1, newRowCount - 1))
        self.surface.axisZ().setLabelFormat('%i')

    def clear(self):
        self.resizeMesh(self.rowCount(), self.columnCount())
